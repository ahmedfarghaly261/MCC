...
interface PanoramaImagesSectionProps {
	images: ImageRecord[];
	loading: boolean;
	processingId: number | null;
	onProcess: (image: ImageRecord) => void;
}

export default function PanoramaImagesSection({
	images,
	loading,
	processingId,
	onProcess,
}: PanoramaImagesSectionProps) {
		...
		<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{images.map((img) => {
				...
				return (
					<div
						key={img.id}
						className="bg-[#1F2937] rounded-lg border border-gray-700/50 hover:border-cyan-500/50 transition-colors"
					>
						<div className="aspect-video bg-linear-to-br from-cyan-900/30 to-blue-900/30 flex items-center justify-center overflow-hidden relative rounded-t-lg">
							{img.download_url ? (
								<img
						...
						<div className="p-4">
							...
							<Button
								type="button"
								variant="outline"
								className={`w-full bg-cyan-500/10 border-cyan-500/50 text-cyan-400 hover:bg-cyan-500/20 ${
									isProcessing
										? "opacity-70 cursor-not-allowed"
										: ""
								}`}
								disabled={isProcessing}
								onClick={() => onProcess(img)}
							>
								{isProcessing ? (
									<>
