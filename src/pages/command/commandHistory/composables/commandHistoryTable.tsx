...
function getCommandName(record: CommandHistoryRecord): string {
  return record.command?.name ?? `CMD-${record.command_id}`;
}
...
function isTelemetryStatus(status: string) {
  return ["telemetry_received", "telemetry_decoded"].includes(status);
}
      ...
      <Table>
        <TableHeader className="bg-card backdrop-blur-sm">
          <TableRow className="border-slate-700/70 hover:bg-transparent">
            ...
            <TableHead className="h-12 px-4 pr-6 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-300">
              Actions
            </TableHead>
          </TableRow>
        </TableHeader>
              ...
              return (
                <Fragment key={record.id}>
                  ...
                  >
                    ...
                    <TableCell className="px-4 pr-6 py-3 text-right">
                      {isTelemetryStatus(record.status) && (
                        <Button
                          size="sm"
                          className="h-8 px-4 text-xs font-medium tracking-wide bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 hover:text-cyan-300 border border-cyan-500/30 transition-all rounded-md flex items-center justify-center gap-1.5 ml-auto"
                          onClick={(event) => {
                              ...
                              <div className="space-y-3 text-sm">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <p className="text-white/90">
                                    {record.command?.description ?? "N/A"}
                                  </p>
                                </div>
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  >
                                    {formatAck(record.command?.requires_ack)}
                                  </Badge>
                                </div>
                              </div>
                            </div>
